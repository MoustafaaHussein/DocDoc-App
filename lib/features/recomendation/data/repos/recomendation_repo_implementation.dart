...
class RecomendationRepoImplementation implements RecomendationRepo {
  ...
  Future<Either<RequestFailure, String>> completeExcersice({
    ...
    try {
      var response = await recomendationRemoteDataSource.completeExcercise(
  ...
  getRecomendationsByEmotions({required String selectedEmotion}) async {
    try {
      var response = await recomendationRemoteDataSource
          .getRecomendationsByEmotions(selectedEmotion: selectedEmotion);
      return right(response);
    } catch (e) {
      if (e is DioException) {
        return Left(ServiceFailure.fromDioError(e));
